...
describe('createPushNotificationsJobs', () => {
  ...
  it('adds jobs to the queue with the correct type', (done) => {
    ...
    testQueue.process('push_notification_code_3', () => {
      expect(
        spyConsole.log.calledWith('Notification job created:', testQueue.testMode.jobs[0].id)
      ).to.be.true;
      done();
    });
  });
  ...
  it('registers the progress event handler for a job', (done) => {
    testQueue.testMode.jobs[0].addListener('progress', () => {
      expect(
        spyConsole.log.calledWith('Notification job', testQueue.testMode.jobs[0].id, '25% complete')
      ).to.be.true;
      done();
    });
  ...
  it('registers the failed event handler for a job', (done) => {
    testQueue.testMode.jobs[0].addListener('failed', () => {
      expect(
        spyConsole.log.calledWith('Notification job', testQueue.testMode.jobs[0].id, 'failed:', 'Failed to send')
      ).to.be.true;
      done();
    });
  ...
  it('registers the complete event handler for a job', (done) => {
    testQueue.testMode.jobs[0].addListener('complete', () => {
      expect(
        spyConsole.log.calledWith('Notification job', testQueue.testMode.jobs[0].id, 'completed')
      ).to.be.true;
      done();
    });
